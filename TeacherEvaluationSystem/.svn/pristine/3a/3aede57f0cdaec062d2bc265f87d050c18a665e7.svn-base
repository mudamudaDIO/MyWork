                ...
                <div class="layui-col-md12">
                    ...
                    -->
                        <div class="layui-card-header">
                            ...
                            <button class="layui-btn" onclick="xadmin.open('添加班级课程','addSubClass',600,400)"><i class="layui-icon"></i>添加</button>
                        </div>
                            ...
                            <table class="layui-table layui-form">
                                ...
                                <tbody>
                     				<c:forEach items="${list}" var="list">
                     					<tr>
                     						<td> <input type="checkbox" value="${list.id}" lay-filter="check" name="" lay-skin="primary"></td>
                     						<td>${list.id}</td>
	                     					<td>${list.classroom.class_name}</td>
	                     					<td>${list.sub.subject_name}</td>
	                     					<td class="td-manage">
		                                      <a title="修改班级课程"  onclick="xadmin.open('修改班级课程','updateSubClass?id=${list.id}',600,400)" href="javascript:;">
    ...
    <script>
      ...
      function member_del(obj,id){
    	  layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
             parm={id:id}
         	 url= "${pageContext.request.contextPath}/admin/delSubClass";
    ...
    //批量删除
      function delAll (argument) {
          ...
          layer.confirm('确认要删除吗？'+ids.toString(),function(index){
              //捉到所有被选中的，发异步进行删除
               $.ajax({
              	url:"${pageContext.request.contextPath}/admin/delMoreSubClass",
   			    data:{ids:ids},
   			    type:"Post",
   			    dataType: "json",
   			  	traditional:true,
