...
function adventure(vote_id,bevote_id) {
	window.location.href="${pageContext.request.contextPath}/admin/lookAnswerInformation?vote_id="+vote_id+"&bevote_id="+bevote_id;
}
</script>
</head>
<body>
                            ...
                            <c:forEach items="${answers}" var="answer">
                                <tr>
                                    <td style="justify-content: center;text-align: left;">
                                     	${answer.teacher.teacher_name}
                                    </td>
									<td>
										<c:forEach items="${teachers}" var="t">
											<c:if test="${t.teacher_id==answer.bevote_id}">
												${t.teacher_name}
											</c:if>
										</c:forEach>
									</td>
									<td>
										<button  class="layui-btn" lay-filter="add" lay-submit="" onclick="adventure(${answer.vote_id},${answer.bevote_id})">查看评价</button>
									</td>
                                </tr>
                            </c:forEach>
                            </table>
