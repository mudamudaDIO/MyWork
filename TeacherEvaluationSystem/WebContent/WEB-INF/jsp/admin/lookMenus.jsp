                            ...
                            <table class="layui-table layui-form">
                                ...
                                <tbody>
                     				<c:forEach items="${menu}" var="m">
                     					<tr>
                     						<td> <input type="checkbox" value="${m.permission_id}" lay-filter="check" name="" lay-skin="primary"></td>
                     						<td>${m.permission_id}</td>
	                     					<td>${m.permission_name}</td>
	                     					<td>${m.url}</td>
	                     					<td>${m.permission_grade}</td>
	                     					<td>${m.paretn_num}</td>
	                     					<td>${m.sort}</td>
	                     					<td>${m.describes}</td>
	                     					
	                     					<td class="td-manage">
		                                      <a title="编辑"  onclick="xadmin.open('编辑','updateMenu?id=${m.permission_id}',600,400)" href="javascript:;">
		                                        <i class="layui-icon">&#xe642;</i>
		                                      </a>
		                                      <a title="删除" onclick="member_del(this,'${m.permission_id}')" href="javascript:;">
		                                        <i class="layui-icon">&#xe640;</i>
		                                      </a>
	                                    	</td>
                                    	</tr>
    ...
    <script>
      ...
      function member_del(obj,id){
    	  layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
             parm={id:id}
         	 url= "${pageContext.request.contextPath}/admin/delMenu";
      ...
      function delAll (argument) {
          ...
          layer.confirm('确认要删除吗？'+ids.toString(),function(index){
              //捉到所有被选中的，发异步进行删除
               $.ajax({
              	url:"${pageContext.request.contextPath}/admin/delMoreMenu",
   			    data:{ids:ids},
   			    type:"Post",
   			    dataType: "json",
   			  	traditional:true,
