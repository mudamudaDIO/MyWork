                ...
                <div class="layui-col-md12">
                    <div class="layui-card">
                        <div class="layui-card-body ">
                        	<div style="float:left">
                           		<div class="layui-inline layui-show-xs-block">
                                    <input type="text" name="username" id="student_text"  placeholder="请输入学号" autocomplete="off" class="layui-input">
                                </div>
                                <div class="layui-inline layui-show-xs-block">
                        ...
                        <div class="layui-card-header">
                            ...
                            <button class="layui-btn" onclick="xadmin.open('添加学生','addStudent',600,400)"><i class="layui-icon"></i>添加</button>
                        </div>
                        <div class="layui-card-body layui-table-body layui-table-main">
                            <table class="layui-table layui-form">
                                ...
                                <c:if test="${student==null}">
	                                <tbody>
	                                	<c:forEach items="${students}" var="stu">
	                     					<tr>
	                     						<td> <input type="checkbox" value="${stu.student_id}" lay-filter="check" name="" lay-skin="primary"></td>
	                     						<td>${stu.student_id}</td>
		                     					<td>${stu.student_name}</td>
		                     					<td>${stu.ins.ins_name}</td>
		                     					<td>${stu.classroom.class_name}</td>
		                     					<td class="td-manage">
			                                      <a title="编辑"  onclick="xadmin.open('编辑','updateStudent?id=${stu.student_id}',600,400)" href="javascript:;">
			                                        <i class="layui-icon">&#xe642;</i>
			                                      </a>
			                                      <a onclick="rePass(${stu.student_id})" title="修改密码" href="javascript:;">
			                                        <i class="layui-icon">&#xe631;</i>
			                                      </a>
			                                      <a title="删除" onclick="member_del(this,'${stu.student_id}')" href="javascript:;">
			                                        <i class="layui-icon">&#xe640;</i>
			                                      </a>
		                                    	</td>
	                                    	</tr>
            					...
            					<c:if test="${student!=null }">
            						<tbody>
	                                	<tr>
	                     					<td> <input type="checkbox" value="${student.student_id}" lay-filter="check" name="" lay-skin="primary"></td>
	                     					<td>${student.student_id}</td>
		                     				<td>${student.student_name}</td>
		                     				<td>${student.ins.ins_name}</td>
		                     				<td>${student.classroom.class_name}</td>
		                     				<td class="td-manage">
			                                    <a title="编辑"  onclick="xadmin.open('编辑','updateStudent?id=${student.student_id}',600,400)" href="javascript:;">
			                                      <i class="layui-icon">&#xe642;</i>
			                                    </a>
			                                    <a onclick="rePass(${student.student_id})" title="修改密码" href="javascript:;">
			                                      <i class="layui-icon">&#xe631;</i>
			                                    </a>
			                                    <a title="删除" onclick="member_del(this,'${student.student_id}')" href="javascript:;">
			                                      <i class="layui-icon">&#xe640;</i>
			                                    </a>
		                                    </td>
	                                    </tr>
    ...
    <script>
      ...
      function member_del(obj,id){
    	  layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
             parm={id:id}
         	 url= "${pageContext.request.contextPath}/admin/delStudent";
      	...
      	/*用户-重置密码*/
	  	function rePass(id){
	  		layer.confirm('确认要重置该学生的密码吗？',function(index){
	              //发异步修改数据
	             parm={id:id}
	         	 url= "${pageContext.request.contextPath}/admin/changeStuPass";
	         	 $.post(url,parm,function(data){
	         		 if(data.flag==1){
      ...
      function delAll (argument) {
          ...
          layer.confirm('确认要删除吗？'+ids.toString(),function(index){
              //捉到所有被选中的，发异步进行删除
               $.ajax({
              	url:"${pageContext.request.contextPath}/admin/delMoreStudent",
   			    data:{ids:ids},
   			    type:"Post",
   			    dataType: "json",
   			  	traditional:true,
      ...
      /*验证上传文件的框*/
		function importUsers(){    	
			...
			var data = formFile;
			$.ajax({
			    url:"${pageContext.request.contextPath}/admin/addMoreStudent",
			    data:data,
			    type:"Post",
			    dataType: "json",
			    async: true,
		...
		/*搜索框*/
      	function search(){
      		...
      		if(text==""){
      			...
      			window.location.href="${pageContext.request.contextPath}/admin/lookstudentById?id="+text;
      		}
